...
export const makePuzzleActions = (game: Phaser.Game): PuzzleActions => {
  return {
    ...
    resetBoard: (puzzle: number[][]) =>
      game.events.emit(BoardActionNames.ResetBoard, puzzle),
    startSolutionPresentation: (solution: number[]) =>
      game.events.emit(BoardActionNames.StartSolutionPresentation, solution),
    cancelSolutionPresentation: () =>
      game.events.emit(BoardActionNames.CancelSolutionPresentation),
  };
};

const colourNumberToString = (colourNumber: number): string =>
  `#${colourNumber.toString(16)}`;
...
class BoardScene extends Phaser.Scene {
  ...
  private createTileLabel(
    value: number,
    color: string
  ): Phaser.GameObjects.Text {
    return this.add
      .text(0, 0, value.toString(), this.textStyle(color))
      .setOrigin(0.5);
  }
  ...
  public create() {
    ...
    this.game.events.on(
      BoardActionNames.StartSolutionPresentation,
      this.onStartSolutionPresentation,
      this
    );
    this.game.events.on(
      BoardActionNames.CancelSolutionPresentation,
      this.onCancelSolutionPresentation,
      this
    );
  ...
  private onShowOverlay() {
    ...
    this.overlay = this.add
      .rectangle(0, 0, width, height, BLACK, 0.6)
      .setOrigin(0, 0)
      .setDepth(1);
    ...
    this.timerEvent = this.time.delayedCall(
      ...
      () => {
        ...
        const tileBackground = this.add.rectangle(
          0,
          0,
          biggerTileWidth,
          biggerTileHeight,
        ...
        const text = this.createTileLabel(
          values[0],
          colourNumberToString(GOLD)
        );
        text.setData("valueIndex", 0);
        this.rotatingTile = this.add
          .container(cx, cy, [tileBackground, tileFace, text])
          .setDepth(2);
  ...
  private createTiles(puzzle: number[][]) {
    ...
    for (const row of range(this.numRows)) {
      for (const col of range(this.numCols)) {
        ...
        const goalValueTileRef = tileRefs.find(
          (tileRef) => tileRef.goalValue === value
        );
        if (!goalValueTileRef) continue;
        const [goalValueRow, goalValueCol] = goalValueTileRef.position;
        const colour =
          (goalValueRow + goalValueCol) % 2 ? ANTIQUE_WHITE : FIREBRICK;
