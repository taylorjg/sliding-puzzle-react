...
export type Move =
  typeof MOVE_UP | typeof MOVE_DOWN | typeof MOVE_LEFT | typeof MOVE_RIGHT;
...
export const makeTiles = (puzzle: Puzzle): Tile[] => {
  ...
  for (const tile of tiles) {
    const calculateTileIndexAtOffset = (
      rowOffset: number,
      colOffset: number
    ): number => {
...
export class BoardManager {
  ...
  getTotalManhattanDistance(): number {
    const manhattanDistances = this.tiles.map((tile) => {
      const goalTileIndex =
        (tile.value === 0 ? this.tiles.length : tile.value) - 1;
  ...
  private getTileToSwapWith(move: Move, blankTile: Tile): Tile {
    switch (move) {
      case MOVE_UP:
        return this.tiles[blankTile.upTileIndex];
      case MOVE_DOWN:
        return this.tiles[blankTile.downTileIndex];
      case MOVE_LEFT:
        return this.tiles[blankTile.leftTileIndex];
      case MOVE_RIGHT:
        return this.tiles[blankTile.rightTileIndex];
      default:
...
export const getChildOfNodeAndMove = (
...
export const solve = async (
  ...
  const ida_star = configure_ida_star(
    h,
    isGoal,
    isNodeInPath,
    successors,
