...
const PuzzleSizeRow: React.FC<PuzzleSizeRowProps> = ({
  solving,
  puzzleSize,
  onChangePuzzleSize,
}) => {
  return (
    <div>
      ...
      <Select
        disabled={solving}
        value={puzzleSize}
        onChange={(e) => onChangePuzzleSize(e.target.value)}
      >
...
const Puzzle: React.FC<PuzzleProps> = ({
  ...
  useEffect(() => {
    ...
    game.events.on(
      BoardEventNames.FinishedPresentingSolution,
      onFinishedPresentingSolution
    );
...
const ButtonsRow: React.FC<ButtonsRowProps> = ({
  ...
  return (
    <PanelRow>
      <Button onClick={() => onReset()} disabled={solving}>
        Reset
      </Button>
      &nbsp;
      <Button onClick={() => onScramble()} disabled={solving}>
        Scramble
      </Button>
      &nbsp;
      {solving ? (
        ...
        <Button disabled={solved} onClick={() => onSolve()}>
          Solve
        </Button>
      )}
    </PanelRow>
...
const getSolvedPuzzle = (puzzleSize: string): number[][] => {
  switch (puzzleSize) {
    ...
    case "4x4":
      return SOLVED_4x4;
    default:
      throw new Error(`unknown puzzleSize ${puzzleSize}`);
  }
};
...
const App = () => {
  ...
  const [puzzleActions, setPuzzleActions] = useState<PuzzleActions | undefined>(
    undefined
  );
      ...
      <MainContent>
        <Panel1>
          <PuzzleSizeRow
            ...
            onChangePuzzleSize={onChangePuzzleSize}
          />
          <PuzzleWrapper>
            <Puzzle
